...
const grupo1 = io.of('/grupo1').on('connection', (socket)=>{
    ...
    console.log('new conncet')
    socket.emit('update_messagens', messages.grupo1)

    socket.on('new_message', (data)=>{
        messages.grupo1.push(data)

        grupo1.emit('update_messagens', messages.grupo1)
    })
})
const grupo2 = io.of('/grupo2').on('connection', (socket)=>{
       
    console.log('new conncet')
    socket.emit('update_messagens', messages.grupo2)

    socket.on('new_message', (data)=>{
        messages.grupo2.push(data)

        grupo2.emit('update_messagens', messages.grupo2)
    })
})
